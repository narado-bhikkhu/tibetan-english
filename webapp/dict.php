...
// 1. Force errors to show in the terminal
...
error_reporting(E_ALL);
  ...
  // Normalize `dictionaries` POST parameter: accept array or comma-separated string
  if (isset($_POST['dictionaries'])) {
    $raw = $_POST['dictionaries'];
    if (is_array($raw)) {
      $dictionaries = $raw;
    } else if (is_string($raw) && trim($raw) !== '') {
      // Allow comma-separated list or single name
      if (strpos($raw, ',') !== false) {
        $parts = preg_split('/\s*,\s*/', trim($raw));
        $dictionaries = $parts;
      } else {
        $dictionaries = array(trim($raw));
      }
    } else {
      $dictionaries = array();
    }
  ...
  if (!empty($dictionaries) && count($dictionaries) > 0) {
    $dictQuery = '';
  ...
  if(isset($_POST['term'])) { // search for the definition of a term
    ...
    while ($row = $results->fetchArray()) {
      ...
      if( $prevDict == $row['dictionary'] && $prevTerm == $row['term'] ) {
        ...
        error_log("RESULT: " . '"' . $row['dictionary'] . '":"' . $definition);
        $defResults[$row['dictionary']] = $definition;
    ...
    error_log(json_encode($defResults));
    ...
    error_log("Listing results starting with: " . $search);
    ...
    print(']');
    error_log("Found " . $count . " results for: " . $search );
  ...
  else if(isset($_POST['getDictionaries'])) {
    header("Content-type: application/json");
    $reqLang = isset($_POST['lang']) ? trim($_POST['lang']) : 'bo';
    if($reqLang == 'en') {
      $dictLang = 'en';
    } else {
      $dictLang = 'bo';
    }
    $statement = $db->prepare('SELECT name FROM DICTNAMES WHERE language = :lang ORDER BY name;');
    $statement->bindValue(':lang', $dictLang, SQLITE3_TEXT);
    $results = $statement->execute();
    $dicts = array();
    if($results == FALSE) {
      die( $db->lastErrorMsg() );
